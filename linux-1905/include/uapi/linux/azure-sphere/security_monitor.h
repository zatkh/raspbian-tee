...
/* N.B. We will remove this structure when existing clients of it go away. */
struct azure_sphere_list_peripherals_v1 {
    // type of peripheral
    uint16_t peripheral_type;
    // buffer size
    uint32_t buffer_size;
...
/* N.B. We will remove the V1 IOCTL variant below when existing clients of it go away. */
#define SECURITY_MONITOR_LIST_PERIPHERALS_V1 _IOWR('p', 0x07, struct azure_sphere_list_peripherals_v1)

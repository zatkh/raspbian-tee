...
#define SECCOMP_MAKE_PATH_DENTRY(_p)                    \
    {                                                   \
        .flags = SECCOMP_OBJFLAG_FS_DENTRY, .path = _p, \
    }

#define SECCOMP_MAKE_PATH_INODE(_p)                                                \
    {                                                                              \
        .flags = SECCOMP_OBJFLAG_FS_INODE | SECCOMP_OBJFLAG_FS_DEVICE, .path = _p, \
    }

#define SECCOMP_MAKE_PATH_MOUNT(_p)                    \
    {                                                  \
        .flags = SECCOMP_OBJFLAG_FS_MOUNT, .path = _p, \
    }

#define SECCOMP_MAKE_PATH_ALL(_p)                                       \
    {                                                                   \
        .flags = SECCOMP_OBJFLAG_FS_DENTRY | SECCOMP_OBJFLAG_FS_INODE | \
                 SECCOMP_OBJFLAG_FS_DEVICE | SECCOMP_OBJFLAG_FS_MOUNT,  \
        .path = _p,                                                     \
    }

#define SECCOMP_MAKE_OBJ_PATH(_c, _p)                                                           \
    {                                                                                           \
        .check = SECCOMP_CHECK_##_c, .type = SECCOMP_OBJTYPE_PATH, .len = 0, .object_path = _p, \
    }

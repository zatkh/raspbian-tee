...
int memdom_create(void){    
    int memdom_id;
    memdom_id = sys_udom_mem_ops(UDOM_CREATE,0,0,NO_UDOM_PRIV_OPS,0);
    if( memdom_id == -1 ){
...
int memdom_kill(int memdom_id){
    ...
    rv = sys_udom_mem_ops(UDOM_KILL,memdom_id,0,NO_UDOM_PRIV_OPS,0);
    if( rv == -1 ){
		fprintf(stderr, "memdom_kill(%d) failed\n", memdom_id);
        return -1;
    }    
...
void *memdom_mmap(int memdom_id,
    ...
    rv = sys_udom_mem_ops(UDOM_MMAP_REG,memdom_id,0,NO_UDOM_PRIV_OPS,0);

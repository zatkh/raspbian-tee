...
int smv_main_init(int global) {
  ...
  rv = sys_udom_ops(INIT,0,NO_UDOM_OPS,0);
  if (rv != 0) {
...
int smv_create(void) {
  ...
  smv_id = sys_udom_ops(CREATE,0,NO_UDOM_OPS,0);
  if (smv_id < 0) {
...
int smv_kill(int smv_id) {
  int rv = 0;
  rv = sys_udom_ops(KILL,smv_id,NO_UDOM_OPS,0);
  if (rv == -1) {
...
int smv_join_domain(int memdom_id, int smv_id) {
  int rv = 0;
  rv = sys_udom_ops(NO_SMV_OPS, smv_id, JOIN, memdom_id);
  if (rv == -1) {
...
int smv_leave_domain(int memdom_id, int smv_id) {
  int rv = 0;
  rv = sys_udom_ops(NO_SMV_OPS, smv_id, LEAVE, memdom_id);
...
int smv_is_in_domain(int memdom_id, int smv_id) {
  int rv = 0;
  rv =  sys_udom_ops(NO_SMV_OPS, smv_id, CHECK, memdom_id);
  if (rv == -1) {
...
int smv_exists(int smv_id) {
  int rv = 0;
  rv =  sys_udom_ops(EXISTS,smv_id,NO_UDOM_OPS,0);
  if (rv == -1) {
...
int smvthread_create(int smv_id, pthread_t* tid, void*(fn)(void*), void* args){
   ...
   * The kernel will set mm->standby_smv_id = smv_id */
   rv = sys_udom_ops(REGISTER,smv_id,NO_UDOM_OPS,0);
  if(rv != 0){
